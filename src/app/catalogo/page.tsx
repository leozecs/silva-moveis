...
export default async function CatalogoPage({ searchParams }: CatalogPageProps) {
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-20"><CatalogView key={`${initialQuery}:${initialCategoryId}`} products={products} categories={categories} initialQuery={initialQuery} initialCategoryId={initialCategoryId} /></section>
    </main>
  );
}
