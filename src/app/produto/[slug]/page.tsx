...
export default async function ProdutoPage({ params }: ProductPageProps) {
  ...
  return (
    <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(productSchema).replace(/</g, "\\u003c") }} /><main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <ProductSelection key={product.id} initialId={product.variants?.[0]?.id ?? ""}><div className="mt-8 grid gap-10 lg:grid-cols-[1.15fr_.85fr] lg:items-start">
        <ProductGallery images={images} name={product.title} variantImages={Object.fromEntries((product.variants ?? []).map((variant) => [variant.id, (variant.images ?? []).map((image) => storefrontImageUrl(image.url)).filter((url): url is string => Boolean(url))]))} />
